...
class ImageRetriever(object):
    ...
    def search_api_request(self):
        # TODO handle tweets block logic

        if self.since_date:
            # TODO extract date and add operator to query
            since = datetime.strptime()
        if self.until_date:
            # TODO extract date and add operator to query
            until = datetime.strptime()

        result = search_api.search(q=query, lang=language, geocode=','.join(self.location_params))

    def _process_result(self, json_response):
        """ Process a block of tweets (update mean sentiments vector). """

        tweets_block = json.loads(json_response)  # refer to https://dev.twitter.com/overview/api/tweets for tweet fields

        for tweet in tweets_block:
            try:
